...
static std::string ansi_colorize(const wcstring &text, const std::vector<highlight_spec_t> &colors)
{
    ...
    return capturer.get_captured_output();
}
...
int main(int argc, char *argv[])
{
    ...
    {
        // TODO: should only need to read from the environment here, no need for parser
        const environment_t &vars = parser_t::principal_parser().vars();
        highlight_shell_no_io(output_wtext, colors, output_wtext.size(), NULL, vars);
    }
